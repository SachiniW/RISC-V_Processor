...
def generate_i(name):
    ...
    if name in SHIFT_IMMEDIATE_INSTRUCTION_NAMES:
        ...
        rs1_decimal = 0
        rs1_binary = "{0:05b}".format(rs1_decimal)
        imm_decimal = 4*np.random.randint(INSTRUCTION_CURRENT+1, INSTRUCTION_CURRENT+10)
        imm_binary = "{0:012b}".format(imm_decimal)
        instruction_binary = imm_binary + rs1_binary + func_instruction + rd_binary + opcode_instruction
        instruction_assembly = format(name, '10s') + "\tx" + str(rd_decimal) + ", x" + str(rs1_decimal) + ", " + str(
            imm_decimal)
...
def generate_s(name):
    ...
    rs2_decimal = random.choice(REGISTERS_TO_USE)
    rs2_binary = "{0:05b}".format(rs2_decimal)
    imm_decimal = 4 * np.random.randint(0, 5)
    imm_binary = "{0:012b}".format(imm_decimal)
...
def generate_sb(name):
    ...
    imm_decimal = (int(INSTRUCTION_CURRENT/4)+2)*4
...
def generate_uj(name):
    ...
    imm_binary = "{0:021b}".format(imm_decimal)

    instruction_binary = imm_binary[0] + imm_binary[10:20] + imm_binary[9] + \
     imm_binary[1:9] + rd_binary + opcode_instruction
    instruction_assembly = format(name, '10s') + "\tx" + str(rd_decimal) + ", " + str(imm_decimal)
...
# # Validaing Input
# while int(TEST_CASES_NUMBER) < 2:
#     TEST_CASES_NUMBER = ('Enter Number of test cases to produce: ')

for test_case in range(int(TEST_CASES_NUMBER)):
    # Initializing all variables
    REGISTERS_NUMBER = 20
    Instructions_Number = 120
    ...
    instruction_memory = open("instruction_memory" + ".mem", "w")
    test = open("test" + ".txt", "w")

    test.write('test' + str(test_case + 1) + '.elf:     file format elf32-littleriscv\n\n\n')
    test.write('Disassembly of section .text:\n\n00000000 <main>:\n')
    hex_file.write("@00000000\n")

    for i in range(Instructions_Number):
        binary_file.write(Instructions_list_binary[i] + "\n")
        instruction_memory.write(instructions_list_hex[i] + "\n")
